...
# macOS 使用系统标题栏，截图中的 y=18 落在内容区；Windows 自绘标题栏
# 则保留原来的标题栏底色断言。
expected_light_bg = "#F2F3F5" if sys.platform == "darwin" else "#ECECEF"
assert bg.name().upper() == expected_light_bg, f"浅色标题栏异常: {bg.name()}"
...
img = win.grab().toImage()
bg = img.pixelColor(500, 18)
expected_dark_bg = "#1E1F24" if sys.platform == "darwin" else "#1A1B20"
assert bg.name().upper() == expected_dark_bg, f"深色标题栏异常: {bg.name()}"
card = next(iter(win._cards.values()))
cimg = card.grab().toImage()
cc = cimg.pixelColor(5, 5)
...
app.processEvents()
img = win.grab().toImage()
assert img.pixelColor(500, 18).name().upper() == expected_light_bg
print("4b. 切回浅色 OK")
...
win.close()
win.shutdown()  # 测试环境未进入 QApplication 退出流程，显式模拟 aboutToQuit 收尾
jobs.shutdown()
settings_store.set_theme_mode("system")
tray_store.save([])
