...
# MiHome-Mac: 米家设备的 macOS 桌面控制端
# Copyright (C) 2026 MiHome-Mac contributors
"""程序入口。
...
_SERVER_NAME = "MiHome-Mac"
_LOCK_NAME = "MiHome-Mac.lock"
...
_BASE_UI_SCALE = 1.0 if sys.platform == "darwin" else 1.25
...
def main() -> int:
    ...
    font = QFont("PingFang SC" if sys.platform == "darwin" else "Microsoft YaHei UI", 9)
    font.setStyleStrategy(QFont.PreferAntialias)
    font.setHintingPreference(QFont.PreferNoHinting)
    app.setFont(font)
    app.setApplicationName("MiHome")
    app.setApplicationVersion(__version__)
    app.setQuitOnLastWindowClosed(False)

    window = MainWindow()
    # macOS 的“退出应用”（例如 ⌘Q 或 Dock 菜单）可直接结束事件循环，
    # 不保证触发主窗口 closeEvent。始终在 aboutToQuit 收尾，避免 PySide
    # 销毁仍在运行的 QThread 时被 Qt 以 SIGABRT 中止。
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 开发态保留报错，打包 app 则显示原生错误对话框。
        _set_console_visible(True)
        import traceback
        traceback.print_exc()
        if getattr(sys, "frozen", False):
            from PySide6.QtWidgets import QMessageBox
            QMessageBox.critical(None, "MiHome 已退出", "应用发生意外错误，请重新打开后重试。")
        code = 1
