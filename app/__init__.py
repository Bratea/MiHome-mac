...
def resource_path(relative: str) -> Path:
    """定位资源文件，兼容开发环境、PyInstaller macOS app 与 Nuitka。

    调用方一律使用相对于项目根的路径，如 ``app/ui/icon.png``。
    PyInstaller 会将随包资源解压到 ``_MEIPASS``；开发环境则回退到
    项目根目录。保留可执行文件相邻目录的兼容分支，便于其他构建器使用。
    """
    ...
    if bundle_root:
        ...
        if candidate.exists():
            return candidate
    exe_candidate = Path(sys.executable).resolve().parent / relative
