...
class JobExecutor(QObject):
    ...
    def submit(
        ...
        if self._shutdown_requested:
            logger.debug("应用正在退出，忽略新的后台任务")
            return
        self._queue.put(_Task(fn, on_success, on_error))

    def shutdown(self) -> None:
        """停止工作线程；可由多个退出路径安全地重复调用。"""
        if self._shutdown_requested:
            return
        ...
        if not self._thread.wait(3000):
            ...
            # 必须等待终止完成：否则 QApplication 清理 QObject 子对象时，
            # Qt 会因“QThread: Destroyed while thread is still running”直接 abort。
            self._thread.wait()
