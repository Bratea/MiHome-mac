...
_RUN_VALUE_NAME = "MiHome-Mac"
_MAC_LAUNCH_AGENT_ID = "com.mihome.mac"


def autostart_supported() -> bool:
    """仅已打包的 Windows/macOS 应用支持开机自启动。"""
    from app import is_packaged

    return is_packaged() and sys.platform in ("win32", "darwin")


def _mac_launch_agent_path() -> Path:
    return Path.home() / "Library" / "LaunchAgents" / f"{_MAC_LAUNCH_AGENT_ID}.plist"


def _mac_app_bundle_path() -> Path | None:
...
def get_autostart() -> bool:
    ...
    if sys.platform == "darwin":
        ...
        bundle = _mac_app_bundle_path()
        return path.exists() and bundle is not None and bundle.exists()
...
def _set_macos_autostart(value: bool) -> None:
    """用当前用户 LaunchAgent 管理已安装 app 的登录启动。"""
    import os
    import plistlib
    import subprocess
